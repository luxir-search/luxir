  ...
  struct FieldSortComparator {
    ...
    bool operator()(const SortDoc& docA, const SortDoc& docB) const {
      ...
      if (cmp != 0) {
        ...
        // For ascending sort: we want to keep the K smallest values, with the largest of those K at the top
        // So we need a max-heap: return true when docA < docB (cmp < 0)
        // For descending sort: we want to keep the K largest values, with the smallest of those K at the top
        // So we need a min-heap: return true when docA > docB (cmp > 0)
        // In both cases, we want the "worst" element at the top for easy replacement
        return cmp < 0;
      }
...
public:
  ...
  void collect(int32_t segment, int32_t docid, float score) {
    ...
    if (pq->size() < topCount) {
      ...
      comparator->setBottom(0); // top of heap (worst element we're keeping)
      if (comparator->compareBottom(docid, segment) < 0) {
        // New doc is better than worst element, so replace it
