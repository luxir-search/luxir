...
public:
  ...
  static inline bool disableExactFilteredMandOptCompositionForTests =
      std::getenv("LUXIR_DISABLE_EXACT_FILTERED_MANDOPT_COMPOSITION") !=
      nullptr;
  ...
  static inline bool disableIntegratedFilteredCountForTests =
      std::getenv("LUXIR_DISABLE_INTEGRATED_FILTERED_COUNT") != nullptr;
  ...
  static inline bool disableExactTermCountForTests =
      std::getenv("LUXIR_DISABLE_EXACT_TERM_COUNT") != nullptr;
  ...
  static inline bool disableCandidateTermFeedForTests =
      std::getenv("LUXIR_DISABLE_CANDIDATE_TERM_FEED") != nullptr;
  ...
  static constexpr int64_t kMaskFilterDensityInverse = luxir::kMaskFilterDensityInverse;
  ...
  class Weight final : public Query::Weight {
    ...
    std::span<Query::Weight*> createWeights(luxir::MemPool& targetPool, Context& context,
  ...
  public:
    ...
    Scorer* createScorer(luxir::MemPool& targetPool, luxir::IndexReader::Segment& segment) override {
      Query::ScorerSupplier* supplier = scorerSupplier(targetPool, segment);
      Query::Demand demand = Query::Demand::fromLeadCost(
          std::numeric_limits<int64_t>::max());
  ...
  class MandOptScorer final : public Query::Scorer {
    ...
    bool optCanExist(int32_t upTo) const {
      ...
      return optDoc != luxir::PostingsReader::END && optDoc <= upTo;
    }
    ...
    int32_t advanceImpacts(int32_t target) {
      ...
      for (;;) {
        ...
        if (windowUpTo >= luxir::PostingsReader::END - 1) {
          return luxir::PostingsReader::END;
        }
        target = windowUpTo + 1;
        moveToNextBlock(target);
        evaluated = true;
    ...
    int32_t advanceInternal(int32_t target) {
      if (target == luxir::PostingsReader::END) {
      ...
      for (;;) {
        ...
        if (reqDoc == luxir::PostingsReader::END || !optIsRequired) {
          id = reqDoc;
          return id;
        }

        int32_t upperBound = reqMaxScore < minCompetitiveScore
          ? luxir::PostingsReader::END
          : windowUpTo;
        ...
        for (;;) {
          ...
          if (optDoc > upperBound) {
            reqDoc = upperBound >= luxir::PostingsReader::END - 1
              ? luxir::PostingsReader::END
              : upperBound + 1;
            goto advanceHead;
          }
          ...
          if (reqDoc == luxir::PostingsReader::END || optDoc == reqDoc) {
            id = reqDoc;
            return id;
          }
        }
    ...
    int32_t nextMatched(int32_t doc) {
      while (doc != luxir::PostingsReader::END && !matches()) {
  ...
  public:
    MandOptScorer(luxir::MemPool& targetPool,
      ...
      unused(targetPool);
      reqMaxScore = mand.scorer->getMaxScoreForSetup(luxir::PostingsReader::END);
    }
    ...
    int32_t next() override {
      assert(id != luxir::PostingsReader::END);
    ...
    int32_t advanceShallow(int32_t target) override {
      ...
      int32_t optDoc = opt.docId();
      if (optDoc != luxir::PostingsReader::END) {
        if (optDoc <= target) {
  ...
  public:
    MandOptBulkScorer(luxir::MemPool& pool, Query::Scorer* mand,
  ...
  public:
    ...
    MandNotScorer(luxir::MemPool& targetPool,
  ...
  private:
    ...
    int32_t doNext() {
      while (id != luxir::PostingsReader::END) {
        if (notid < id) {
  ...
  class ConjunctionScorer final : public Query::Scorer {
    ...
    int32_t advanceTarget(int32_t target) {
      ...
      for (;;) {
        if (target >= luxir::PostingsReader::END - 1) {
          return luxir::PostingsReader::END;
        }
        skipCount(SkipStats::conjRangeEvals);
        int32_t evalStartTarget = target;
        int32_t upTo = luxir::PostingsReader::END;
        ...
        if (competitive && std::isfinite(maxScore)
          ...
          if (competitive) {
            int32_t refinedUpTo = luxir::PostingsReader::END;
            for (auto* scorer : scorers) {
        ...
        if (competitive) {
          ...
          if (failStreak > 0 && upTo < luxir::PostingsReader::END - 1) {
            ...
            int64_t maxDoc = (int64_t) luxir::PostingsReader::END - 1;
        ...
        failStreak = 0;
        skipSeen = true;
        if (upTo >= luxir::PostingsReader::END - 1) {
          return luxir::PostingsReader::END;
        }
        target = upTo + 1;
      }
    ...
    int32_t leadTo(int32_t id) {
      for (;;) {
        ...
        if (pruned == luxir::PostingsReader::END) {
          return luxir::PostingsReader::END;
        }
  ...
  public:
    ...
    ConjunctionScorer(luxir::MemPool& pool, std::span<Query::Scorer*> allScorers,
    ...
    int32_t next() override {
      assert(docid != luxir::PostingsReader::END);
  ...
  public:
    ...
    static inline int32_t denseThresholdInverseForTests = [] {
      const char* value = std::getenv("LUXIR_DENSE_THRESHOLD_INVERSE");
      return value != nullptr ? std::atoi(value) : kDenseThresholdInverse;
    }();
    static inline int32_t denseLeapfrogThresholdForTests = [] {
      const char* value = std::getenv("LUXIR_DENSE_LEAPFROG_THRESHOLD");
    ...
    static inline int32_t termLeadLeapfrogThresholdForTests = [] {
      const char* value =
          std::getenv("LUXIR_TERM_LEAD_LEAPFROG_THRESHOLD");
    ...
    static inline bool disableCandidateCountRetainForTests =
        std::getenv("LUXIR_DISABLE_CANDIDATE_COUNT_RETAIN") != nullptr;
  ...
  private:
    ...
    int32_t batchCompactByRemainingBound(
        int32_t size, double remaining) LUXIR_INLINE {
      for (int32_t i = 0; i < size; i++) {
        boundDrops[(size_t) i] =
    ...
    int32_t LUXIR_INLINE applyScoredProbeToCandidates(
    ...
    int32_t LUXIR_INLINE applyScorerToCandidates(
    ...
    int32_t scoreNextWindowImpl(ScoreWindow& out, DocSet* filter, int32_t min, int32_t max,
      ...
      for (;;) {
        if (leadDoc == luxir::PostingsReader::END) {
          out.max = max;
          return luxir::PostingsReader::END;
        }
        if (leadDoc >= max) {
  ...
  public:
    ConjunctionBulkScorer(luxir::MemPool& pool, std::span<Query::Scorer*> scorers,
                          std::span<Query::Scorer*> prohibitedScorers,
  ...
  class DisjunctionScorer final : public Query::Scorer {
    ...
    luxir::IndirectPQ<ApproxSlot, decltype(idComparator)> pq;
    ...
    static std::span<ApproxSlot> makeMembers(
        luxir::MemPool& pool, std::span<Scorer*> scorers,
        std::span<const uint8_t> twoPhaseScorers,
        bool enableTwoPhase) {
    ...
    static std::span<ApproxSlot*> makePointers(luxir::MemPool& pool,
  ...
  public:
    ...
    DisjunctionScorer(luxir::MemPool& pool, std::span<Scorer*> scorers,
                      std::span<const uint8_t> twoPhaseScorers,
                      bool enableTwoPhase)
            : clauses(scorers),
              members(makeMembers(
    ...
    int32_t approximationNext() override {
      ...
      docid = pq.top().next();
      for (;;) {
        if (docid == luxir::PostingsReader::END) {
          pq.removeTop();
          if (pq.size() == 0) {
            break;
          }
    ...
    int32_t approximationAdvance(int32_t target) override {
      ...
      while (pq.size() > 0 && pq.top().docId() < target) {
        if (pq.top().advance(target) == luxir::PostingsReader::END) {
          pq.removeTop();
        } else {
          pq.updateTop();
        }
      }
      docid = pq.size() > 0 ? pq.top().docId() : luxir::PostingsReader::END;
      return docid;
    }
    ...
    int32_t advanceShallow(int32_t target) override {
      int32_t upTo = luxir::PostingsReader::END;
      for (auto& member : members) {
        if (member.docId() != luxir::PostingsReader::END) {
    ...
    float getMaxScore(int32_t upTo) override {
      ...
      for (auto& member : members) {
        ...
        if (doc != luxir::PostingsReader::END && doc <= upTo) {
  ...
  class MaxScoreDisjunctionScorer final : public Query::Scorer {
    ...
    luxir::IndirectPQ<Scorer, decltype(idComparator)>* pq = nullptr;
    ...
    void rebuildGlobalHeap() {
      ...
      pq = pool.make<luxir::IndirectPQ<Scorer, decltype(idComparator)>>(essentialPointers, essentialCount);
    }
    ...
    int32_t nextGlobal() {
      ...
      for (;;) {
        if (pq->size() == 0) {
          docid = luxir::PostingsReader::END;
          currentScore = 0.0f;
          return docid;
        }
        int32_t topDoc = pq->top().docId();
        if (topDoc == luxir::PostingsReader::END) {
          pq->removeTop();
          continue;
        }
        if (topDoc <= currid) {
          int32_t nextDoc = pq->top().next();
          if (nextDoc == luxir::PostingsReader::END) {
            pq->removeTop();
          } else {
            pq->updateTop();
          }
  ...
  public:
    ...
    MaxScoreDisjunctionScorer(luxir::MemPool& pool, std::span<Scorer*> scorers,
  ...
  private:
    ...
    void configureDisjConjCount(luxir::MemPool& pool, bool enable) {
    ...
    bool lessWindowOrder(int32_t a, int32_t b) const {
      ...
      // raw-max order for two and three clauses: there it preserves Luxir's
  ...
  public:
    // The passed in span of scorers will be modified (rearranged).
    MaxScoreBulkScorer(luxir::MemPool& pool, std::span<Query::Scorer*> scorers,
  ...
  public:
    MinShouldMatchWandScorer(luxir::MemPool& pool, std::span<Scorer*> scorers, int32_t minMatch)
            : scorers(scorers),
              clauseMax(pool.make_arr<float>(scorers.size()), scorers.size()),
  ...
  public:
    MinShouldMatchScorer(luxir::MemPool& pool, std::span<Scorer*> costAscendingScorers, int32_t minMatch)
