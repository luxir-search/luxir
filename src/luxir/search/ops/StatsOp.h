  ...
  class Calc : public Calculator {
    ...
    void emitResult(double result) {
      ...
      getTarget(nullptr, [&](luxir::api::Val& val) {
        routeTarget<luxir::api::ArrDouble>(val, mr, [&](auto& target) {
          using Target = std::remove_cvref_t<decltype(target)>;
          if constexpr (std::is_same_v<Target, luxir::api::Val>) {
