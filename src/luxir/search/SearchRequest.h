...
// --- Request/response proto model (concrete luxir::api, non-owning) ---
...
// build-by-backing (see luxir/api/build.h) into the SearchResponse's own monotonic arena
// (`mr`), then it is serialized (luxir::api::encode / write_json) and dropped. Every
// luxir::api type is trivially destructible, so the response arena is released wholesale
...
using ReqProto = luxir::api::SearchRequest;
using RespProto = luxir::api::SearchResponse;

// Short alias for the response build-by-backing helpers (luxir/api/build.h).
...
struct ExecutionProfilePieceState {
  luxir::api::ExecutionProfilePiece wire;
...
public:
  ...
  luxir::api::DocFormat docFormatDefault = luxir::api::DocFormat::COLUMNS;
...
// storage backed by this object's `mr` (build-by-backing, see luxir/api/build.h).  For
...
public:
  ...
  // wholesale with the arena. All luxir::api types are trivially destructible.
  ArenaResource mr;
  ...
  static SearchResponse* create(SearchRequest& req, bool last = true) {
    ...
    return luxir::arenaCreate<SearchResponse>(*arena, req, *arena, last);
  }

  // named columns, part of DocList or part of a FacetResult.
  using ColumnsType = decltype(std::declval<luxir::api::DocList>().columns);
};
