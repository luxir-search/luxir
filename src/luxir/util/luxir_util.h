...
// Example:
//    auto cleaner = luxir::scope_guard([](){ luxir::Signal::unlisten("mergeStart");});  // lambda is moved
//    auto cb = [](){ luxir::Signal::unlisten("mergeStart");
//    auto cleaner2 = luxir::scope_guard(cb);  // lambda is not moved, only referenced.
...
// change.
//
// use LUXIR_UNALIGNED_START struct X{} LUXIR_UNALIGNED_END;
// gcc and msvc have different ways of spelling this :-(
#ifdef __GNUC__
#define LUXIR_UNALIGNED_START ;
#define LUXIR_UNALIGNED_END __attribute__((__packed__, __aligned__(1)))
#endif

#ifdef _MSC_VER
#define LUXIR_UNALIGNED_START __pragma(pack(push,1))
#define LUXIR_UNALIGNED_END __pragma(pack(pop))
#endif

#if defined(__GNUC__)
#  define LUXIR_INLINE __attribute__((always_inline))
#  define LUXIR_NOINLINE __attribute__((noinline))
#  define LUXIR_RESTRICT __restrict
#elif defined(_MSC_VER)
#  define LUXIR_INLINE __forceinline
#  define LUXIR_NOINLINE __declspec(noinline)
#  define LUXIR_RESTRICT __restrict
#else
#  define LUXIR_INLINE
#  define LUXIR_NOINLINE
#  define LUXIR_RESTRICT
#endif
...
#else
template <class T>
LUXIR_INLINE inline T loadUnaligned(const void* p) {
  T value;
  memcpy(&value, p, sizeof(value));
  return value;
}
template <class T>
LUXIR_INLINE inline void storeUnaligned(void* p, T value) {
