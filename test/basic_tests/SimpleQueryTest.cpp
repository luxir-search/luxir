...
public:
  ...
  void SetUp() override {
    helper.index(flatdoc("id", "d1", "title_un", "Blade Runner",
                         "body_un", "a replicant story", "tag_s", "scifi"),
                 UpdateMessage::NO_COMMIT);
    helper.index(flatdoc("id", "d2", "title_un", "The Running Man",
                         "body_un", "arnold runs fast", "tag_s", "action"),
                 UpdateMessage::NO_COMMIT);
    helper.index(flatdoc("id", "d3", "title_un", "Bladed Weapons",
                         "body_un", "swords and knives", "tag_s", "scifi"),
...
TEST_F(SimpleQueryTest, numericFieldSyntaxDegradesWithWarning) {
  ...
  auto run = [&](std::string_view q, std::string_view warnCode) {
    auto req = localReq(helper.getSearchEngine());
    req->collection("main").topDocs("q").simpleQuery(q, {"title_un"}).fields({"id"}).limit(-1);
    req->execute();
...
TEST_F(SimpleQueryTest, phraseAndPrefix) {
  auto docs = search("\"blade runner\"", {"title_un"});
  ASSERT_EQ(1u, docs.size());
  EXPECT_TRUE(hasId(docs, "d1"));

  // phrase must not match reordered terms
  EXPECT_EQ(0u, search("\"runner blade\"", {"title_un"}).size());

  // last-token prefix, explicit: runn* matches runner/running/runs
  docs = search("runn*", {"title_un", "body_un"});
...
TEST_F(SimpleQueryTest, fuzzyTerm) {
  ...
  auto docs = search("bladee~1", {"title_un"});
  EXPECT_EQ(2u, docs.size());
  EXPECT_TRUE(hasId(docs, "d1"));
  EXPECT_TRUE(hasId(docs, "d3"));
}
...
TEST_F(SimpleQueryTest, quotedValueOnStringFieldIsExactMatch) {
  ...
  helper.index(flatdoc("id", "d4", "title_un", "extra doc", "tag_s", "in stock"),
               UpdateMessage::COMMIT);
  auto docs = search("tag_s:\"in stock\"", {"title_un"});
  ASSERT_EQ(1u, docs.size());
...
TEST_F(SimpleQueryTest, emptyStringMatchesNothing) {
  EXPECT_EQ(0u, search("", {"title_un"}).size());
  EXPECT_EQ(0u, search("+ | -", {"title_un"}).size());
}

TEST_F(SimpleQueryTest, wholeInputStarMatchesAll) {
  EXPECT_EQ(3u, search("*", {"title_un"}).size());
}
