...
class SoluxTestListener : public testing::EmptyTestEventListener {
  ...
  void OnTestStart(const testing::TestInfo& test_info) override {
    ...
    rng_seed = (suiteHash<<32) + testnameHash;  // these are currently 32 bit hashes (from Hash::hash) so combine by shifting.
...
int main(int argc, char **argv) {
  ...
  testing::AddGlobalTestEnvironment(new SoluxEnvironment());
